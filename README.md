...
`[linter]` can be one of `autopep8`, `black`, `dotnet_format`, `erblint`, `eslint`, `flake8`, `gofmt`, `golint`, `isort`, `mypy`, `oitnb`, `php_codesniffer`, `prettier`, `pylint`, `rubocop`, `stylelint`, `swift_format_official`, `swift_format_lockwood`, `swiftlint` and `xo`:
